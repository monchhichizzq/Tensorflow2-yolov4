...
class Yolo_loss():
    def __init__(self, anchors, num_classes, ignore_thresh=.5, label_smoothing=0.1):
    ...
    def loss(self, y_true, y_pred):
        ...
        for l in range(num_layers):
            ...
            if self.label_smoothing:
                true_class_probs = _smooth_labels(true_class_probs, self.label_smoothing)

            # 将yolo_outputs的特征层输出进行处理
            # grid为网格结构(13,13,1,2)，raw_pred为尚未处理的预测结果(m,13,13,3,85)
            # 还有解码后的xy，wh，(m,13,13,3,2)
            grid, raw_pred, pred_xy, pred_wh = yolo_head(yolo_outputs[l],
                                                         self.anchors[anchor_mask[l]], self.num_classes, input_shape,
                                                         calc_loss=True)
